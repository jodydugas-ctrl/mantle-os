...
- **Mantle doctrine:** `documents/grimoire/` contains the single canonical Grimoire — a Grimoire 2.0 file with a Mantle OS environment binding (§9). The `documents/Mantle_*.md` and `documents/guides/` files are implementation doctrine, guides, audits, and onboarding material; they explain and interpret the current reference body but are not additional Grimoire editions.
...
but modified only through their owning package manager. After Pass 34 committed
`examples/tests/package-lock.json`, the inventory still classified it as a test file eligible for
semantic chunk review.

Changes:

- Added lockfile path recognition to the optimization inventory.
- Classified `package-lock.json` and common package-manager locks as `M lockfile`.
- Marked lockfiles as generated by the owning package manager, inventory-only, and ineligible for
  semantic chunk rewriting.
- Strengthened `OPT-1` to prove the demo-smoke lockfile is categorized and dispositioned under the
  lockfile rule.

Deletion decision: no files or features were deleted. The previous examples/tests fallback served
test-surface classification, but lockfiles have stricter package-manager ownership and should not be
queued for manual semantic optimization.

Proof path: `PYTHONPATH=src python -m mantle check`.

## Pass 34 Receipt

Function served: the online Demo Smoke workflow repeatedly stalled before any Mantle browser test
ran, inside dependency setup. The workflow needed deterministic dependency installation and bounded
steps so CI failures become receipted evidence instead of indefinite pending state.

Changes:

- Added an `examples/tests/package-lock.json` lockfile generated by npm.
- Narrowed the root `package-lock.json` ignore rule so the demo-smoke lockfile can be committed.
- Switched Demo Smoke setup from fresh `npm install` resolution to cached `npm ci`.
- Added workflow and step timeouts around install, static serving, browser smoke, and jsdom smoke.

Deletion decision: no files or tests were deleted. The old unlocked install path served demo-test
dependency setup, but deterministic `npm ci` preserves that function while reducing CI entropy and
making hangs auditable.
