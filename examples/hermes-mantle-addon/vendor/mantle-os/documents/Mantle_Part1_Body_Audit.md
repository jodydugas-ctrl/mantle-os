...
**Mantle OS** · Certify the Zombie Body before any MIND is fused
...
`python -m mantle audit` runs the Stage-1 Zombie Body gate and `python -m mantle prove` runs the
current security invariant suite red/green (the tamper proofs `audit --break-hash/primer/seal` must each
exit non-zero). Fill every `Pass?` cell; any open hard-fail blocks Phase 2. Checks cover the
breakage axis (B-rows + hard-fails HF-Bxx), the self-regulating tissue (SELF / NOC / MEMW — §A1.13), and the
...
## A1.13 — Body tissue *(self/other identity · nociception · graded memory)*

| #      | Requirement | Pass? | Notes |
|--------|-------------|-------|-------|
| SELF-1 | The genesis key is minted ONCE and is absent from `boot_order`, the assembled snapshot, and every cube band | | `src/mantle/core/body.py` |
...
ZOMBIE BODY CERTIFICATION
  ...
  Security invariants: ____ / current suite green   (python -m mantle prove)
