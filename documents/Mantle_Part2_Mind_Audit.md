...
and the full **88 invariants**, then Pass B tests the fusion (the M-rows + the Phase-2
...
| Check | Pass? | Notes |
|-------|-------|-------|
| All Stage 1 rows re-run and PASS on the fused organism (and `prove` -> 88/88) | | If any regressed, the fusion is wrong — fix the fusion, not the Body |
