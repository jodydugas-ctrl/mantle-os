# Mantle OS — Homeostatic AppAI Framework: Architecture Note

**Mantle OS** · What the shape is, and what each part owes the others.
...
What it adds, without weakening any of that:
...
`mind/runtime.py` — **AppAIRuntime**, the surface a coding agent uses to *be* the
...
`src/mantle/assimilator/` implements Path B: a read-only AST **scanner** classifies every symbol of
