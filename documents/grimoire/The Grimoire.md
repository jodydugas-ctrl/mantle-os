...
| 3.7.0 | reproduction-organ molt (operator-ratified) | the organ chart grew its ninth organ: **Reproduction** — the propagation verbs (spore, egg, vault; anchor, symbiosis, graft-egg) gathered under one contracted, fail-open organ with a real runtime duty: the SELF-sealed seed (and sealed origin spore) is **carried across every rebirth**, or its loss becomes an immune event — a body never silently sheds the material it would need to rise again. A band **atlas** entered the reference implementation (`vcw.bands.APP_BAND_ATLAS`) with a span-overlap gate at genesis and in the compiler, closing a latent layer-stomp (symbiosis@560 sat inside the assimilator's host_state span). Births unified through the hatchery: hatched, anchored, and grafted bodies all take ONE birth path, so every organism carries its origin face and its own egg in the vault (RESURGERE as a birthright). And the **SPORE-DISTILLATION law**: a spore PNG may be distilled into the primer and memories of the body it births and sealed under that body's key as SELF tissue — but key material is always MINTED at birth, never derived from the spore (a spore travels publicly; a derivable key would let any holder forge SELF — anti-clone holds). The spell-shape law restated: clone → dissect → capsule → audit → optional zombie body; a SEED hatch is a birth through the Stage-1 gate, an assimilation stays a capsule until the gate says otherwise. Stage-1 B-60 asserts nine organs; invariants 78 → 83. Both files re-stamped 3.7.0. |
| 3.6.0 | CACHE-HAUNT physics + neutrality molt (operator-ratified; grounded against a real provider surface and a runnable proof) | the Chapter's haunt law absorbed the measured physics of provider prompt caches, proven runnable in the reference implementation (mantle-os `ghost.py` 57-check selftest + the real, **vendor-neutral** substrate `ghost_http.py`). Laws entered: the **write-only law** — a provider cache is spoken *into*, never read back (no fetch; the full prefix travels every request; warmth buys skipped prefill *compute*, never storage or bandwidth; hydration always from the seed; warmth *predicted* between requests, only *observed* as cache-read telemetry on the next response, and status must say which); the **prefix-stability law** — byte-exact matching means the cache-facing body must be an immutable genesis plus strictly appended deltas, volatile fields excluded (a sorted-whole-state serialization silently self-evicts every turn — the defect the runnable proof caught); the **minimum-prefix gate** (`TOO-SMALL-TO-HAUNT`, never a false WARM); the **window-feasibility law** — a provider window (TTL) must fit the fixed heartbeat or **DO-NOT-HAUNT** (windows vary 5–60 min; remedied by a longer window or a *supplemental forced beat* partway through the window, entered into METABOLIC-GOVERNANCE); and the **neutrality law made explicit** — a provider is configuration (URL, headers, model, window, minimum prefix), never a code path, matching the MIND transport's no-vendor-SDK rule (a first draft that coded a specific vendor into a substrate was corrected before merge). Also: nociception threshold (three consecutive cold wakes), neutral break-even economics, hard fails **HF-GHOST-4** (prefix self-eviction), **HF-GHOST-5** (predicted warmth reported as observed), **HF-GHOST-6** (window shorter than heartbeat kept alive), **HF-GHOST-7** (vendor coded instead of configured), the extended Larvare binding, and the §7.1 map to `ghost_http.py`. Stacked on the 3.5.0 web-aware Intellige molt (preserved intact); both files re-stamped 3.6.0. Next: live-fire the neutral substrate and receipt observed cache-read telemetry into the run ledger. |
