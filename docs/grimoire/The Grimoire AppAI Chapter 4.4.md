...
- Python package root: `mantle/`. CLI entry: `python -m mantle <command>` (`mantle/cli.py`, `mantle/lineage_cli.py`).

### F.1 Spell → implementation

| Spell | Primary code | Docs / examples / CLI |
|---|---|---|
| `ANIMARE` (birth from egg) | `mantle/egg.py`, `mantle/hatchery.py`, eggs in `eggs/*.json` | `docs/v3/Organism_Lifecycle.md`; CLI `hatch` |
| `NECROMANCY` (assimilate / anchor a host) | `mantle/assimilator/` (`scanner.py`, `scanner_ts.py`, `organ_map.py`, `report.py`, `wrappers.py`), `mantle/anchor.py`, `mantle/graft.py`, `mantle/symbiosis.py` | `Mantle_Assimilator.md`, `docs/v3/Assimilation_Guide.md`; `examples/sample_app/`; CLI `anchor`, `graft` |
| `VITALS-CHECKUP` (diagnostics) | `mantle/doctor.py`, `mantle/audits/` (`stage1.py`, `invariants.py`) | `docs/v3/Audit_Guide.md`, `Mantle_Part1_Body_Audit.md`; CLI `vitals`, `doctor` |
| `MEM-DIGESTION` (ingest / quarantine OTHER) | `mantle/ingestion.py`, `mantle/mem.py`, `mantle/vcw/metabolism.py`, `mantle/core/redact.py`, `mantle/organs/immune.py` | CLI `feed` |
| `SKILL-CALCIFICATION` (harden reflex) | `mantle/hatchery.py` (INSTINCTS gauntlet: sandbox → trial → calcify), `mantle/compiler.py`, `mantle/phenotype.py`, `mantle/teach.py` | `examples/phenotype_demo.py` |
| `METABOLIC-GOVERNANCE` (energy / cost / wake) | `mantle/organs/heart.py`, `mantle/symbiosis.py`, `mantle/vcw/metabolism.py`, `mantle/mind/runtime.py` | `docs/Mantle_Urge_System.md`, `docs/Mantle_VCW_Tiers.md` |
| `CREMATION` (retire / DNR) | policy-enforced, no single module: `mantle/vault.py` (DNR & reconstruction policy), `mantle/organs/immune.py` (tombstone / redact), `mantle/lineage_cli.py` (lineage) | `docs/MOLT_LEDGER.md` |
| `VOCARE` (MIND readiness, Phase 2) | `mantle/mind/` (`mind.py`, `containment.py`, `runtime.py`, `inner_voice.py`, `transport.py`), `mantle/audits/stage2.py`, `mantle/organs/brain.py` | `Mantle_Part2_Mind.md`, `Mantle_Part2_Mind_Audit.md`; CLI `mind`, `audit-mind` |
| `RESURGERE` (reconstruct from seed) | `mantle/vault.py` (seed → `reconstruct()`), `mantle/hatchery.py` (`incubate`), `mantle/lineage_cli.py` | `docs/Mantle_v3_Migration.md`, `docs/MOLT_LEDGER.md` |

### F.2 Egg & assimilation (the vital substrate)

Birth and assimilation share **one substrate, two casts**:

- **Egg (declarative birth):** an AppAI declared entirely as data — `mantle/egg.py`; incubated to a certified organism by `mantle/hatchery.py`; sample eggs in `eggs/calculator.json`, `eggs/greeter.json`, `eggs/notes_graft.json`.
- **Assimilation (raise an existing app):** read-only host dissection in `mantle/assimilator/scanner.py` (plus `scanner_ts.py` for TS/JS) → assimilation map `organ_map.py` → Phase-0 inventory artifacts `report.py` → fail-open hook runtime `wrappers.py` (where `mantle_touch`, `mantle_sense`, `mantle_display`, `mantle_state_write`, `mantle_external_call` live) → residency via `mantle/anchor.py` and graft eggs via `mantle/graft.py`.
- **Stage-1 gate (the same gate for both paths):** `mantle/audits/stage1.py` with `mantle/audits/invariants.py`; the hard-fail codes (HF-B32, HF-B40, HF-MIND, …) are enforced here.

### F.3 Organs → modules

| Organ (§A.3) | Module |
|---|---|
| Heart | `mantle/organs/heart.py` |
| Genome | `mantle/organs/genome.py` |
| Nervous System | `mantle/organs/nervous.py` |
| Senses | `mantle/organs/senses.py` |
| Immune | `mantle/organs/immune.py` |
| Limbs | `mantle/organs/limbs.py` |
| Memory | `mantle/organs/memory.py` |
| Brain | `mantle/organs/brain.py` |
| Organ contract | `mantle/organs/contract.py`; atlas `docs/Mantle_Organ_Atlas.md`, `docs/v3/Organ_Contracts.md` |

### F.4 VCW cube, bands & hooks

- **Cube substrate:** `mantle/vcw/cube.py`, `png.py`, `entry.py`, `indexes.py`, `drivers.py`; metabolism / compaction `mantle/vcw/metabolism.py`. Guides: `docs/v3/VCW_Guide.md`, `docs/Mantle_VCW_Tiers.md`, `examples/vcw/`.
- **Band plan (§E.4):** `mantle/vcw/bands.py` — boot sectors, driver registry, the reserved band genome, and capacity thresholds.
- **Fail-open hooks (§E.5):** `mantle/assimilator/wrappers.py`. **MIND containment (§A.5):** `mantle/mind/containment.py` (the MIND may write only `thoughts` / `brain`). **Limb dispatch & Action Execution Proof (§E.5):** `mantle/organs/limbs.py`.
