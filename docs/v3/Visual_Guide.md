# Mantle OS v3 — Visual Guide

*Pictures for humans, text-native diagrams for agents. Everything in `docs/assets/`.*
...
2. **SVG diagrams** — v3-accurate, versionable, diffable, rendered by GitHub, and
   **readable by coding agents as plain XML text**. When art and SVG disagree, the SVG
   (and ultimately the code) wins.

---

## The v3 diagrams (normative-ish: kept in sync with the code)

### Anatomy of the organism mesh
![Mantle OS v3 anatomy](../assets/mantle_v3_anatomy.svg)
...
The two renders live in `docs/assets/` as `mantle_poster.png` and `vcw_cube_render.png`.

### The Mantle OS poster (`docs/assets/mantle_poster.png`)

![Mantle OS poster (v2-era)](../assets/mantle_poster.png)

**Still true in v3:** body-first order; the doctrine sequence (Primer → grow → audit →
...
- v3 added things the poster predates: the **SignalBus** with enforced **organ
  ...
  agents.
- "Immunization / memory reweighting" on the poster is doctrine-level; in v3 code the
...
### The voxel cube render (`docs/assets/vcw_cube_render.png`)
...
- Binary art lives in `docs/assets/`, referenced from docs with a caption that dates it
