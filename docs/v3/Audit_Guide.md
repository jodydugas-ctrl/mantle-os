...
python -m mantle prove         # the 32 security invariants (red/green)
...
## Stage 1 — the Zombie Body gate (`mantle/audits/stage1.py`)
...
## Stage 2 — the MIND gate (`mantle/audits/stage2.py`)
...
## The invariants (`mantle/audits/invariants.py`)

32 red/green guards, each proving a hard-fail fires (and, where relevant, that the
...
and say so in the commit). The v3 history has examples of both, documented in
`docs/Mantle_v3_Migration.md`.
