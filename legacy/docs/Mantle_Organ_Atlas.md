...
**Mantle OS · Gen-4** · The first-class organ taxonomy
...
- **Safe-reuse:** only entry-addressed (`log-json`/`keyvalue`) layers are reclaimable; spatial and
  exec layers are never recycled while referenced (`mantle/vcw/cube.py::Cube.compact`).
...
- **Learned skills (learning → instinct):** the MIND may cultivate and prove code; once it
  ...
  (hash- + capability-gated; `mantle/vcw/drivers.py::ExecDriver`). The first such skill is the **Inner
