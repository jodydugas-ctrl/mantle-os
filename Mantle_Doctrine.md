...
> Source texts for this doctrine live in [`doctrine/`](doctrine/):
...
1. **The Primer lives in the BODY, not the cube.** There are two distinct "genomes": the
   **agent genome** (Primer + commandments + defining data) is held by the **Body** (which may
   ...
   The cube is pure *experiential* memory. (See `vcw/GUIDE.md` and `vcw/body.py`.)
...
Everything else in the v2.0 docs stands. The working code in [`vcw/`](vcw/) already embodies
