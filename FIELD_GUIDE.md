...
```bash
python -m mantle graft eggs/notes_graft.json examples/sample_app
```
```python
res = graft.apply(load_graft("eggs/notes_graft.json"), host)   # workspace; host untouched
...
Start from `eggs/greeter.json` and change five things:
