...
*The organism re-certifies on every commit: the Stage-1 gate, 96 security invariants, the
Stage-2 gate, and three tamper proofs that show the audit CATCHES violations.*

**Current certification count:** 96 security invariants (`python -m mantle prove`).
...
python -m mantle prove         # 96 security invariants, red/green
